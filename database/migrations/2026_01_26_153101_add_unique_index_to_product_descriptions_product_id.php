<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
  /**
   * Run the migrations.
   */
  public function up(): void
  {
    // Drop and recreate the table - much faster than TRUNCATE
    Schema::dropIfExists('product_descriptions');

    Schema::create('product_descriptions', function (Blueprint $table) {
      $table->id();
      $table->integer('product_id')->unique(); // Add unique constraint from the start
      $table->string('name1_de');
      $table->string('name1_fr');
      $table->string('name1_it');
      $table->string('name2_de');
      $table->string('name2_fr');
      $table->string('name2_it');
      $table->text('text1_de')->nullable();
      $table->text('text1_fr')->nullable();
      $table->text('text1_it')->nullable();
      $table->text('text2_de')->nullable();
      $table->text('text2_fr')->nullable();
      $table->text('text2_it')->nullable();
      $table->string('image1')->nullable();
      $table->string('image2')->nullable();
      $table->string('image3')->nullable();
      $table->string('image4')->nullable();
      $table->string('image5')->nullable();
      $table->string('image6')->nullable();
      $table->string('alt1')->nullable();
      $table->string('alt2')->nullable();
      $table->string('alt3')->nullable();
      $table->string('alt4')->nullable();
      $table->string('alt5')->nullable();
      $table->string('alt6')->nullable();
      $table->timestamps();
    });
  }

  /**
   * Reverse the migrations.
   */
  public function down(): void
  {
    // Drop and recreate without the unique constraint
    Schema::dropIfExists('product_descriptions');

    Schema::create('product_descriptions', function (Blueprint $table) {
      $table->id();
      $table->integer('product_id'); // No unique constraint
      $table->string('name1_de');
      $table->string('name1_fr');
      $table->string('name1_it');
      $table->string('name2_de');
      $table->string('name2_fr');
      $table->string('name2_it');
      $table->text('text1_de')->nullable();
      $table->text('text1_fr')->nullable();
      $table->text('text1_it')->nullable();
      $table->text('text2_de')->nullable();
      $table->text('text2_fr')->nullable();
      $table->text('text2_it')->nullable();
      $table->string('image1')->nullable();
      $table->string('image2')->nullable();
      $table->string('image3')->nullable();
      $table->string('image4')->nullable();
      $table->string('image5')->nullable();
      $table->string('image6')->nullable();
      $table->string('alt1')->nullable();
      $table->string('alt2')->nullable();
      $table->string('alt3')->nullable();
      $table->string('alt4')->nullable();
      $table->string('alt5')->nullable();
      $table->string('alt6')->nullable();
      $table->timestamps();
    });
  }
};
